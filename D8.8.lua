...
for _, soundName in ipairs({"AntiFreakClickSound", "AntiFreakMenuSound"}) do
    local oldSound = SoundService:FindFirstChild(soundName)
    if oldSound then
        oldSound:Destroy()
    end
end
...
local menuSound = New("Sound", {
    Name = "AntiFreakMenuSound",
    SoundId = "rbxasset://sounds/electronicpingshort.wav",
    Volume = 0.28,
    PlaybackSpeed = 0.86,
})
...
local function PlayUISound(kind)
    ...
    local sound = kind == "menu" and menuSound or clickSound
    pcall(function()
...
local function CreateFeaturePanel(parent, title, descriptionEN, descriptionRU, defaultValue, callback)
    ...
    local description = New("TextLabel", {
        Position = UDim2.fromOffset(40, 43),
    ...
    local content = New("Frame", {
        Position = UDim2.fromOffset(40, 79),
    ...
    local function renderToggle(animated)
        ...
        local knobColor = value and Color3.new(1, 1, 1) or Theme.SubText
        local knobPos = value and UDim2.new(1, -18, 0, 3) or UDim2.fromOffset(3, 3)
        if animated then
    ...
    local function recalc(animated)
        ...
        if expanded then
            targetHeight = 89 + layout.AbsoluteContentSize.Y + 10
        end
...
local function AddMiniToggle(parent, labelEN, labelRU, defaultValue, callback)
    ...
    local function render(animated)
        ...
        local kp = value and UDim2.new(1, -16, 0, 3) or UDim2.fromOffset(3, 3)
        if animated then
...
local AimHighlight
local AIM_RENDER_NAME = "AntiFreakHubAimRenderV6"
...
local function AimRender(dt)
    ...
    FOVRing.Visible = Config.Aim and Config.ShowFOV and Config.CurrentProfile == nil
    ...
    AimTouchButton.Visible = UserInputService.TouchEnabled and Config.CurrentProfile == nil

    if not Config.Aim or Config.CurrentProfile ~= nil then
...
local function StartOrbit()
    ...
    OrbitConnection = RunService.RenderStepped:Connect(function(dt)
        if not Config.TargetOrbit or Config.CurrentProfile ~= nil then return end
        local localRoot = GetRoot()
...
-- Hub MM2 card
local mm2Card = CreateCard(HubPage, 72)
...
BindTheme(mm2Desc, {TextColor3 = "SubText"})
BindDetail(mm2Desc, "Opens the clean MM2 profile and temporarily disables universal modules.", "Открывает отдельный чистый профиль MM2 и временно отключает универсальные функции.")
local mm2Open = New("TextButton", {
    AnchorPoint = Vector2.new(1, 0.5),
...
Round(mm2Open, 11)
mm2Open.Parent = mm2Card
BindTheme(mm2Open, {BackgroundColor3 = "Surface3", TextColor3 = "Text"})
AnimateButton(mm2Open)

-- AIM is not a separate profile anymore; everything lives under this arrow.
local aimContent
local AimFeatureController
AimFeatureController, aimContent = CreateFeaturePanel(
    HubPage,
    "Aim",
    "Aim stays inside Hub. Expand this row to reveal aim, FOV and target settings.",
    "AIM остаётся внутри Hub. Раскройте эту строку, чтобы увидеть настройки наведения, FOV и Target.",
    false,
    nil
)

AimController = AddMiniToggle(aimContent, "Aim Assist", "Включить AIM", Config.Aim, function(state)
    SetAim(state)
end)
AddMiniSlider(aimContent, "Aim Response", "Резкость наведения", 20, 100, Config.AimSharpness, function(value)
    Config.AimSharpness = value
end, "%")
AddMiniSlider(aimContent, "FOV Radius", "Радиус FOV", 45, 320, Config.FOVRadius, function(value)
    Config.FOVRadius = value
end, " px")
AddMiniToggle(aimContent, "Show FOV", "Показывать FOV", Config.ShowFOV, function(state)
    Config.ShowFOV = state
end)
AddMiniSlider(aimContent, "FOV Opacity", "Прозрачность FOV", 5, 95, Config.FOVOpacity, function(value)
    Config.FOVOpacity = value
end, "%")
AddMiniSlider(aimContent, "FOV Thickness", "Толщина FOV", 1, 5, Config.FOVThickness, function(value)
    Config.FOVThickness = value
end)
AddMiniSlider(aimContent, "Max Distance", "Макс. дистанция", 100, 2500, Config.AimMaxDistance, function(value)
    Config.AimMaxDistance = value
end, " studs")
AddMiniToggle(aimContent, "Wall Check", "Проверка стен", Config.AimWallCheck, function(state)
    Config.AimWallCheck = state
end)
AddMiniToggle(aimContent, "Team Check", "Проверка команды", Config.AimTeamCheck, function(state)
    Config.AimTeamCheck = state
end)
AddMiniToggle(aimContent, "Target Highlight", "Подсветка цели", Config.AimHighlight, function(state)
...
AddMiniAction(aimContent, "Target Part", "Часть тела", Config.AimTargetPart == "Head" and "HEAD" or "ROOT", function(button)
    if Config.AimTargetPart == "Head" then
...
-- Player selector embedded in AIM details.
local selectorRow = New("Frame", {
    Size = UDim2.new(1, 0, 0, 62),
    BackgroundColor3 = Theme.Surface3,
    BorderSizePixel = 0,
})
Round(selectorRow, 11)
selectorRow.Parent = aimContent
...
Round(targetSelectorList, 11)
targetSelectorList.Parent = aimContent
...
local function RefreshTargetList()
    ...
    for index, player in ipairs(list) do
        ...
        row.MouseButton1Click:Connect(function()
            ...
            targetSelectorList.Visible = false
            AimFeatureController:Recalculate()
            PlayUISound("click")
        end)
    end
end
...
targetSelectorButton.MouseButton1Click:Connect(function()
    RefreshTargetList()
    targetSelectorList.Visible = not targetSelectorList.Visible
    AimFeatureController:Recalculate()
    PlayUISound("click")
end)

TargetOrbitController = AddMiniToggle(aimContent, "Orbit Target", "Вращаться вокруг цели", Config.TargetOrbit, function(state)
    if state then
        StartOrbit()
    else
        StopOrbit(true)
...
AddMiniSlider(aimContent, "Orbit Radius", "Радиус вращения", 2, 12, Config.TargetOrbitRadius, function(value)
    Config.TargetOrbitRadius = value
end, " studs")
AddMiniSlider(aimContent, "Orbit Speed", "Скорость вращения", 1, 6, math.floor(Config.TargetOrbitSpeed + 0.5), function(value)
    Config.TargetOrbitSpeed = value
end)
AddMiniToggle(aimContent, "Target Ring", "Кольцо вокруг цели", Config.TargetRing, function(state)
    Config.TargetRing = state
    RefreshTargetRing()
end)
AddMiniAction(aimContent, "Teleport Once", "Телепортироваться к цели", "GO", function()
    local localRoot = GetRoot()
    local targetRoot = SelectedTargetPlayer and SelectedTargetPlayer.Character and SelectedTargetPlayer.Character:FindFirstChild("HumanoidRootPart")
...
AddMiniToggle(InterfaceContent, "UI Sounds", "Звуки интерфейса", Config.UISounds, function(state)
    Config.UISounds = state
end)
...
BindDetail(MM2CenterDesc, "MM2 profile is active. Universal modules are temporarily disabled.", "Профиль MM2 активен. Универсальные функции временно отключены.")
...
local function DisableUniversalModules()
    ...
    if AimController then AimController:Set(false, true) else SetAim(false) end
    if TimerController then TimerController:Set(false, true) end
    if Config.TargetOrbit then
        StopOrbit(true)
        if TargetOrbitController then TargetOrbitController:Set(false, false) end
    end
end
...
local function RestoreUniversalState()
    ...
    if SavedUniversalState.Aim and AimController then AimController:Set(true, true) end
...
local function OpenProfile(profileName)
    if profileName ~= "MM2" or Config.CurrentProfile then return end
    SaveUniversalState()
    DisableUniversalModules()
    Config.CurrentProfile = "MM2"
    ...
    if Config.Animations then
        ...
        Tween(ProfileContainer, 0.72, {GroupTransparency = 0}, Enum.EasingStyle.Quint)
        Tween(MM2Center, 0.90, {Position = UDim2.fromScale(0.5, 0.5)}, Enum.EasingStyle.Back)
    else
...
local function ExitProfile()
    if Config.CurrentProfile ~= "MM2" then return end
...
-- Startup compatibility: AIM is no longer a separate profile/tab system.
local function SwitchAimTab()
end
...
print("[AntiFreak Hub] Compact v6 build finished loading")
