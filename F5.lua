...
--// State Registry for Feature Backup & Restoration
local StateRegistry = {
	ESP = false,
	Fly = false,
	Speedhack = false,
...
--// ==================== VISUALS TAB (ESP & SPECTRUM PALETTE) ====================
...
-- LocalScript (StarterPlayer -> StarterPlayerScripts) - PART 3 (Continuation)
...
mobileControlsFrame.Size = UDim2.new(0, 70, 0, 150)
mobileControlsFrame.Position = UDim2.new(1, -85, 0.5, -75)
...
-- Flight RenderStepped Engine (PC Keyboard + Mobile Thumbstick/Buttons)
RunService.RenderStepped:Connect(function()
	...
	-- Mobile Touch Controls & Virtual Dynamic Thumbstick Hook
	if mobileUpPressed then moveDir += Vector3.new(0, 1, 0) end
	if mobileDownPressed then moveDir -= Vector3.new(0, 1, 0) end
	if hum.MoveDirection.Magnitude > 0 then
		local lookH = Vector3.new(camera.CFrame.LookVector.X, 0, camera.CFrame.LookVector.Z).Unit
		local rightH = Vector3.new(camera.CFrame.RightVector.X, 0, camera.CFrame.RightVector.Z).Unit
		moveDir += (lookH * -hum.MoveDirection.Z) + (rightH * hum.MoveDirection.X)
	end
