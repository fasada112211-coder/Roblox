...
CreateSection(AimSettingsPage, "Aim", "Fast target selection and camera lock.")
CreateSection(AimTargetPage, "Target", "Select a player and control local target movement.")
...
local function AimRenderStep(deltaTime)
    ...
    if Config.AimSharpness >= 95 then
        ...
        local response = math.clamp(Config.AimSharpness / 100, 0.05, 0.94)
        camera.CFrame = camera.CFrame:Lerp(targetCFrame, response)
    end
end
...
SafeModule("Aim Engine", function()
    ...
    AimController = CreateToggle(
        AimSettingsPage,
        "Aim Assist",
        "Snap quickly to the closest target inside FOV.",
        Config.Aim,
        function(state)
            Config.Aim = state
            if not state then
    ...
    CreateSlider(AimSettingsPage, "Aim Response", 20, 100, Config.AimSharpness, function(value)
        Config.AimSharpness = value
    end, "%", "100% gives an instant camera snap; lower values keep a small amount of easing.")

    CreateSlider(AimSettingsPage, "Max Distance", 100, 2500, Config.AimMaxDistance, function(value)
        Config.AimMaxDistance = value
    end, " studs", "Maximum world distance at which a player can become an aim target.")
...
local TargetSelectorCard = nil
local TargetSelectorButton = nil
local TargetSelectorList = nil
local TargetSelectedLabel = nil
local TargetTeleportController = nil
local SelectedTargetPlayer = nil
local TargetBillboard = nil
local TargetTeleportConnection = nil

local function RemoveTargetBillboard()
...
local function RefreshTargetBillboard()
    RemoveTargetBillboard()
    ...
    Round(ring, 999)
    Stroke(ring, ProfileThemes.AIM.Accent, 0.08, 2)
    ...
    local dot = New("Frame", {
        ...
        Position = UDim2.fromScale(0.5, 0.5),
        Size = UDim2.fromOffset(8, 8),
        BackgroundColor3 = ProfileThemes.AIM.Accent,
        BorderSizePixel = 0,
    })
    Round(dot, 999)
...
local function SetSelectedTarget(player)
    ...
    if TargetSelectedLabel then
        if player then
            TargetSelectedLabel.Text = "Selected  ·  " .. player.Name
        else
            TargetSelectedLabel.Text = "Selected  ·  None"
        end
    end
    ...
    RefreshTargetBillboard()
end

local function StopTargetTeleport()
...
local function StartTargetTeleport()
    StopTargetTeleport()

    local elapsed = 0
    TargetTeleportConnection = RunService.Heartbeat:Connect(function(dt)
        if not Config.TargetTeleport or Config.CurrentProfile ~= "AIM" then
            return
        end
        ...
        if not localRoot or not targetRoot or not humanoid or humanoid.Health <= 0 then
            return
        end

        localRoot.CFrame = targetRoot.CFrame * CFrame.new(0, 0, Config.TargetDistance)
...
local function RefreshTargetList()
    if not TargetSelectorList then
        return
    end
    ...
    for index, player in ipairs(players) do
        ...
        BindTheme(row, {BackgroundColor3 = "Surface3", TextColor3 = "Text"})
        AnimateButton(row)

        row.MouseButton1Click:Connect(function()
            SetSelectedTarget(player)
            TargetSelectorList.Visible = false
            if TargetSelectorCard then
                Tween(TargetSelectorCard, 0.32, {Size = UDim2.new(1, 0, 0, 70)}, Enum.EasingStyle.Quint)
            end
        end)
    end
end

SafeModule("Target System", function()
    TargetSelectorCard = CreateCard(AimTargetPage, 70)
    ...
    Round(infoButton, 8)
    infoButton.Parent = TargetSelectorCard
    BindTheme(infoButton, {BackgroundColor3 = "Surface3", TextColor3 = "SubText"})
    AnimateButton(infoButton)
    ...
    title.Parent = TargetSelectorCard
    BindTheme(title, {TextColor3 = "Text"})

    TargetSelectedLabel = New("TextLabel", {
    ...
    TargetSelectedLabel.Parent = TargetSelectorCard
    BindTheme(TargetSelectedLabel, {TextColor3 = "SubText"})
    ...
    TargetSelectorButton.Parent = TargetSelectorCard
    BindTheme(TargetSelectorButton, {BackgroundColor3 = "Surface3", TextColor3 = "Text"})
    AnimateButton(TargetSelectorButton)

    TargetSelectorList = New("ScrollingFrame", {
        Position = UDim2.fromOffset(40, 58),
        Size = UDim2.new(1, -52, 0, 126),
        BackgroundTransparency = 1,
        BorderSizePixel = 0,
        ScrollBarThickness = 2,
        ScrollBarImageColor3 = Theme.Accent,
    ...
    TargetSelectorList.Parent = TargetSelectorCard

    local listLayout = New("UIListLayout", {
        Padding = UDim.new(0, 5),
        SortOrder = Enum.SortOrder.LayoutOrder,
    })
    listLayout.Parent = TargetSelectorList

    local explanation = New("TextLabel", {
        Position = UDim2.fromOffset(40, 50),
        Size = UDim2.new(1, -52, 0, 16),
        BackgroundTransparency = 1,
        Text = "Choose a player from the current server list.",
        TextXAlignment = Enum.TextXAlignment.Left,
        TextSize = 8,
        Font = Enum.Font.GothamMedium,
        TextColor3 = Theme.SubText,
    ...
    explanation.Parent = TargetSelectorCard
    BindTheme(explanation, {TextColor3 = "SubText"})

    local explained = false
    infoButton.MouseButton1Click:Connect(function()
        explained = not explained
        explanation.Visible = explained
        infoButton.Text = explained and "v" or ">"
        if not TargetSelectorList.Visible then
            Tween(TargetSelectorCard, 0.32, {Size = UDim2.new(1, 0, 0, explained and 78 or 70)}, Enum.EasingStyle.Quint)
        end
    end)
    ...
    TargetSelectorButton.MouseButton1Click:Connect(function()
        ...
        Tween(TargetSelectorCard, 0.34, {
            Size = UDim2.new(1, 0, 0, TargetSelectorList.Visible and 194 or (explained and 78 or 70)),
        }, Enum.EasingStyle.Quint)
    end)

    TargetTeleportController = CreateToggle(
        AimTargetPage,
        "Target Teleport",
        "Continuously keeps your local character a short distance behind the selected player.",
        Config.TargetTeleport,
        function(state)
            Config.TargetTeleport = state
            if state then
                StartTargetTeleport()
            else
                StopTargetTeleport()
    ...
    CreateSlider(
        ...
        " studs",
        "Distance kept between your character and the selected target while Target Teleport is enabled."
    )

    CreateToggle(
        AimTargetPage,
        "Target Ring",
        "Show a local visual ring around the selected player's character.",
        Config.TargetRing,
        function(state)
    ...
    CreateAction(
        AimTargetPage,
        "Teleport Once",
        "Teleport your local character behind the selected player one time.",
        "GO",
        function()
            local localRoot = GetRoot()
    ...
    Players.PlayerRemoving:Connect(function(player)
        if player == SelectedTargetPlayer then
            ...
            Config.TargetTeleport = false
            if TargetTeleportController then
                TargetTeleportController:Set(false, false)
            end
            StopTargetTeleport()
        end
    end)
    ...
    Players.PlayerAdded:Connect(function()
        if TargetSelectorList and TargetSelectorList.Visible then
            RefreshTargetList()
        end
    end)
end)
...
print("[AntiFreak Hub] Compact v5 build finished loading")
